...
### Original code taken from [Anthropic's Computer Use](https://github.com/anthropics/anthropic-quickstarts/blob/main/computer-use-demo/README.md); app built on top of that.
...
## Development - Recommended (using pre-build docker image)

This docker run command mounts the repo inside the docker image, such that you can edit files from the host. Streamlit is already configured with auto reloading.
...
## Development - Rebuild Docker Image

Same thing as above, but now you're rebuilding your Docker img which may take a while.

```bash
./setup.sh  # configure venv, install development dependencies, and install pre-commit hooks
docker build . -t computer-use-demo:local  # manually build the docker image
export ANTHROPIC_API_KEY=%your_api_key%
docker run \
    -e ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY \
    -v $(pwd)/computer_use_demo:/home/computeruse/computer_use_demo/ `# mount local python module for development` \
...
Once inside the container's shell, you can view the log files:
```
cat /tmp/streamlit_stdout.log
```
Or, if you prefer to continuously monitor the logs:
```
